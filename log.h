...
#define LOGE(__fmt__, ...)                          \
  do {                                              \
    if (LOG_LEVEL >=  LVL_ERROR) {                  \
      LOG(LOG_ERROR_PREFIX __fmt__ RTT_CTRL_RESET, ##__VA_ARGS__); \
    }                                               \
  } while (0)
...
#define LOGW(__fmt__, ...)                            \
  do {                                                \
    if (LOG_LEVEL >=  LVL_WARNING) {                  \
      LOG(LOG_WARNING_PREFIX __fmt__ RTT_CTRL_RESET, ##__VA_ARGS__); \
    }                                                 \
  } while (0)
...
#define LOGI(__fmt__, ...)                         \
  do {                                             \
    if (LOG_LEVEL >=  LVL_INFO) {                  \
      LOG(LOG_INFO_PREFIX __fmt__ RTT_CTRL_RESET, ##__VA_ARGS__); \
    }                                              \
  } while (0)
...
#define LOGD(__fmt__, ...)                          \
  do {                                              \
    if (LOG_LEVEL >=  LVL_DEBUG) {                  \
      LOG(LOG_DEBUG_PREFIX __fmt__ RTT_CTRL_RESET, ##__VA_ARGS__); \
    }                                               \
  } while (0)
...
#define LOGV(__fmt__, ...)                            \
  do {                                                \
    if (LOG_LEVEL >=  LVL_VERBOSE) {                  \
      LOG(LOG_VERBOSE_PREFIX __fmt__ RTT_CTRL_RESET, ##__VA_ARGS__); \
    }                                                 \
  } while (0)
...
/*
 * NOTE: Below features are out of maintaince.
 */
...
#define SE_CALL(x) \
  do {             \
    if ((x)->result != 0) { ERROR_ADDRESSING(); } } while (0)
